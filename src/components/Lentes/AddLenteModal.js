...
const AddLenteModal = ({
  ...
  const uploadImageToCloudinary = async (imageUri) => {
    ...
    try {
      ...
      const filename = imageUri.split('/').pop() || `lente_${Date.now()}.jpg`;
      ...
      // Usar el preset específico para lentes
      ...
      if (!result.canceled && result.assets) {
        ...
        try {
          ...
          if (validUrls.length > 0) {
            ...
            console.log(`${validUrls.length} imágenes subidas exitosamente`);
          } else {
  ...
  /**
   * Guardar lente - Las imágenes ya son URLs de Cloudinary
   */
  const handleSave = () => {
    if (!validate()) {
      return;
