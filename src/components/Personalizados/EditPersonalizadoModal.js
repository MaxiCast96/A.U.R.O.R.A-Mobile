...
const EditPersonalizadoModal = ({
  ...
  useEffect(() => {
    if (personalizado && visible) {
      const editData = {
        ...
        categoria: personalizado.categoria || '',
        marcaId: personalizado.marcaId?._id || personalizado.marcaId || '',
        material: personalizado.material || '',
        color: personalizado.color || '',
        tipoLente: personalizado.tipoLente || '',
        precioCalculado: personalizado.precioCalculado?.toString() || '',
        fechaEntregaEstimada: personalizado.fechaEntregaEstimada ? 
          new Date(personalizado.fechaEntregaEstimada).toISOString().split('T')[0] : '',
        cotizacion: {
          total: personalizado.cotizacion?.total?.toString() || '',
          validaHasta: personalizado.cotizacion?.validaHasta ? 
            new Date(personalizado.cotizacion.validaHasta).toISOString().split('T')[0] : ''
        }
      };
  ...
  const handleSave = () => {
    if (!personalizado) {
      Alert.alert('Error', 'No hay personalizado seleccionado para editar');
      return;
    }
    ...
    >
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Editar Producto Personalizado</Text>
          <TouchableOpacity 
        ...
        <View style={styles.actionButtons}>
          ...
          >
            ...
            <Text style={styles.saveButtonText}>Actualizar Personalizado</Text>
          </TouchableOpacity>
        </View>
        ...
        {showDatePicker && (
          <DateTimePicker
            value={form.fechaEntregaEstimada || form.cotizacion.validaHasta ? 
              new Date(showDatePicker === 'entrega' ? form.fechaEntregaEstimada : form.cotizacion.validaHasta) : 
              new Date()}
            mode="date"
...
export default EditPersonalizadoModal;